...
function getFeed(type) {
    ...
    .then((response) => {
        ...
        response.feed.forEach(element => {
            if (element.image == 1) {
                feed+='<hr class = "hr"><div class="item"><p style="margin: 2px; color: white"; >'+element.content+'</p><img src=http://stardash.de:2000/image/'+element.image_path.replaceAll(" ","SPACESYMBOL")+' style="width: 100%; max-height: auto; border-radius: 10px"><br></div>'
